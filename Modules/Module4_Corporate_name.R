...
for(i in 1:nrow(wb))   {  
  ...
  res<-data.frame(str_detect(test,"Name/corporate name|Name/ corporate name"))
  colnames(res)<-"Result"
  ...
  if ( nrow(res) > 0 ){
    for (j in  nrow(res):1){ 
      ...
      if (str_detect(test[res2],":")){
        ...
        for (jj in 1: 10){ 
          ...
          if (substr(test[res2+jj+1], nchar(test[res2+jj+1]), nchar(test[res2+jj+1]))==":" & nchar(test[res2+jj+1])<52){
            break
            }
            if (str_detect(test[res2+jj+1],"Basis for opinion")){
              break
            }
            if (str_detect(test[res2+jj+1],"Scope of the review")){
              break
            }
            if (str_detect(test[res2+jj+1],"Information about the authorised medicinal product")){
              break
            }  
    ...
    start <- stringr::str_which(lines, "Name/corporate name")
    if (length(start) == 1 ){
    for (j in 1:length(start)) {
      if (substr(lines[start[j]], nchar(lines[start[j]]), nchar(lines[start[j]]))==":"){
        test <- lines[start[j]:(start[j]+30)]
        res<-data.frame(str_detect(test,"Name/corporate name"))
        colnames(res)<-"Result"
        res<-subset(res,res$Result==TRUE)
